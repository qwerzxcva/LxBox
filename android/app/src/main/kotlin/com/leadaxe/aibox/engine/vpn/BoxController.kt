 ...
 * either fires an `Intent` at the [AIVpnService] (which then owns the engine)
...
class BoxController(
    ...
    fun start(activity: Activity): Boolean {
        ...
        val intent = Intent(activity, AIVpnService::class.java)
    ...
    fun startFromBackground() {
        val intent = Intent(context, AIVpnService::class.java)
            .setAction(AIVpnService.ACTION_CONNECT)
